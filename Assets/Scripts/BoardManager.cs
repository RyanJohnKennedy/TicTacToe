...
public class BoardManager : MonoBehaviour
{
    ...
    private void Draw()
    {
        ...
        {
            for (int z = 0; z < boardState.Height; z++)
            {
                if(boardState.Board[x,z] == Tile.EMPTY)
                {
                    GameObject go = Instantiate(emptyTile, new Vector3(x * 1.1f, 0f, z * 1.1f), Quaternion.identity);
                    go.GetComponent<TileController>().position = new Vector2Int(x, z);
                }
                else if (boardState.Board[x, z] == Tile.CROSS)
                {
                    Instantiate(crossTile, new Vector3(x * 1.1f, 0f, z * 1.1f), Quaternion.identity);
                }
                else if (boardState.Board[x, z] == Tile.NOUGHT)
                {
                    Instantiate(noughtTile, new Vector3(x * 1.1f, 0f, z * 1.1f), Quaternion.identity);
                }
            }
        }
    }
